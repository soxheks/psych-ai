...
- 侧栏场景、心情开场选项、语音和发送按钮具有悬停及点击反馈。首页和心灵随笔入口在桌面端显示为带说明的导航块，在手机端收为图标按钮；页面跳转使用项目统一转场。心情选项仅填入草稿，用户发送后才请求 AI。
- 欢迎内容在开始聊天后收起，给聊天记录留出空间。减少动态效果时关闭位移、涟漪和入场动画，保留文字及按键反馈。
- 回复文字由现有 Django `POST /api/chat/` 接口提供。逐字效果在收到完整回复后由浏览器呈现，并非后端流式接口。
- 语音默认开启。进入 AI 对话页约 0.9 秒后，角色会切换到回答动作并朗读页面暖心开场白；如果手机浏览器拦截自动播放，会提示用户轻触页面并在第一次交互时补播。点击扬声器图标可关闭。优先从浏览器已有声音中选择晓伊、瑶瑶、晓晓、慧慧等中文声线，音调 1.22、语速 1.02、音量 0.9；没有中文声音或朗读失败时仍正常显示文字。没有新增收费语音服务。
...
默认测试地址是 `http://127.0.0.1:8000`，可通过 `PREVIEW_URL` 覆盖。
